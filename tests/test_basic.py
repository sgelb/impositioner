...
class test_core(unittest.TestCase):
    ...
    def test_cut_in_signatures(self):
        self.assertEqual(list(core.cut_in_signatures([1, 2, 3, 4], 2)),
                         [[1, 2], [3, 4]])
        self.assertEqual(list(core.cut_in_signatures([1, 2, 3, 4], 4)),
                         [[1, 2, 3, 4]])
        self.assertEqual(list(core.cut_in_signatures([1, 2, 3, 4], 1)),
                         [[1], [2], [3], [4]])
        self.assertEqual(list(core.cut_in_signatures([1, 2, 3, 4], 3)),
                         [[1, 2, 3], [4]])

    def test_calculate_scaled_sub_page_size(self):
        self.assertEqual(
            core.calculate_scaled_sub_page_size(2, core.paperformats["a5"]),
            [298, 420])
        self.assertEqual(
            core.calculate_scaled_sub_page_size(4, core.paperformats["a5"]),
            [210, 298])
        self.assertEqual(
            core.calculate_scaled_sub_page_size(2, core.paperformats["a4"]),
            [421, 595])
        self.assertEqual(
            core.calculate_scaled_sub_page_size(8, core.paperformats["a4"]),
            [298, 210])
    ...
    def test_add_blanks(self):
        ...
        self.assertEquals(core.add_blanks(self.portrait_pdf, 2),
                          self.portrait_pdf)

        blank = core.create_blank_copy(self.portrait_pdf[0])
        result = core.add_blanks(self.portrait_pdf, 4)
        self.assertEquals(len(result), len(self.portrait_pdf) + 4)

        self.assertEquals(result[9], self.portrait_pdf[9])
        self.assertEquals(result[12], self.portrait_pdf[10])

        for i in [-1, -2, 10, 11]:
            self.assertEquals(result[i], blank)

    def test_get_media_box_size(self):
        self.assertEqual(core.get_media_box_size(self.portrait_pdf),
                         [420, 595])
        self.assertEqual(core.get_media_box_size(self.landscape_pdf),
                         [595, 420])

        pages = list(self.portrait_pdf)
        pages[0].Rotate = 90
        self.assertEqual(core.get_media_box_size(pages),
                         [595, 420])
    ...
    def test_calculate_margins(self):
        ...
        self.assertEqual(core.calculate_margins([50, 80], [50, 10]),
                         (1, 0, 35))
        self.assertEqual(core.calculate_margins([80, 50], [10, 50]),
                         (1, 35, 0))
        with self.assertRaises(ZeroDivisionError):
    ...
    def test_validate_papersize(self):
        self.assertEquals(core.validate_papersize(None, "mm"), None)
        self.assertEquals(core.validate_papersize("a5", "mm"),
                          core.paperformats["a5"])
        self.assertEquals(core.validate_papersize("17.5x200", "mm"),
                          [50, 567])
        self.assertEquals(core.validate_papersize("17.50x200.000", "mm"),
                          [50, 567])
        self.assertEquals(core.validate_papersize(".50x10", "cm"),
                          [14, 283])
        self.assertEquals(core.validate_papersize(".50X10", "cm"),
                          [14, 283])
        with self.assertRaises(SystemExit):
    ...
    def test_validate_pages_per_sheet(self):
        self.assertEquals(core.validate_pages_per_sheet(2), 2)
        self.assertEquals(core.validate_pages_per_sheet(4), 4)
        self.assertEquals(core.validate_pages_per_sheet(8), 8)
        self.assertEquals(core.validate_pages_per_sheet(16), 16)
        with self.assertRaises(SystemExit):
    ...
    def test_validate_signature_length(self):
        self.assertEquals(core.validate_signature_length(4), 4)
        self.assertEquals(core.validate_signature_length(8), 8)
        self.assertEquals(core.validate_signature_length(40), 40)
        self.assertEquals(core.validate_signature_length(-1), -1)
        with self.assertRaises(SystemExit):
    ...
    def test_create_filename(self):
        self.assertEquals(core.create_filename("foo/bar.tmp"),
                          "booklet.bar.tmp")
        self.assertEquals(core.create_filename("/foo/bar.tmp"),
                          "booklet.bar.tmp")
        self.assertEquals(core.create_filename("~/bar.tmp"),
                          "booklet.bar.tmp")
